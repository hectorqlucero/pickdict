...
(defn -main []
  ...
  (let [db {:dbtype "sqlite" :dbname "test_crud.db"}]
    ...
    (println "ORDERS table exists:" (pick/table-exists? db "ORDERS"))
    (println "ORDERS_DICT table exists:" (pick/table-exists? db "ORDERS_DICT"))
    ...
    ;; Define computed fields
    (println "About to define dictionary fields...")
    (pick/define-dictionary-field db "ORDERS_DICT" "CUSTOMER_NAME" "T" "1" "TCUSTOMER;FULL_NAME" "Customer Name")
    (println "Defined CUSTOMER_NAME")
    (pick/define-dictionary-field db "ORDERS_DICT" "PRODUCT_NAMES" "T" "2" "TPRODUCT;NAME" "Product Names")
    (println "Defined PRODUCT_NAMES")
    (pick/define-dictionary-field db "ORDERS_DICT" "LINE_TOTALS" "C" "" "MULTIPLY:QUANTITIES,UNIT_PRICES" "Line Totals")
    (println "Defined LINE_TOTALS")
    (pick/define-dictionary-field db "ORDERS_DICT" "SUBTOTAL" "C" "" "SUM:LINE_TOTALS" "Subtotal")
    (println "Defined SUBTOTAL")    ;; Create sample data
